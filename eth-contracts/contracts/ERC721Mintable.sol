...
contract Ownable {
    ...
    modifier onlyOwner() {
        require( _owner == msg.sender   , "Caller is not the owner of the contract");
        _;
    }
    //  4) fill out the transferOwnership function
    
    //  5) create an event that emits anytime ownerShip is transfered (including in the constructor)
    event OwnershipTransfered(address theNewOwner);
...
contract Pausable is Ownable {
    ...
    function pauseSetter(bool pauseValue) public onlyOwner {
        _paused = pauseValue;
    }
...
//    @dev Approves another address to transfer the given token ID
    function approve(address to, uint256 tokenId) public {
        ...
        require(msg.sender == owner || isApprovedForAll(owner,msg.sender),"Account needs to be the owner or approved party");
...
contract RealEstateERC721Token is ERC721Metadata("Real Estate Marketplace", "REM", "https://s3-us-west-2.amazonaws.com/udacity-blockchain/capstone/") {
